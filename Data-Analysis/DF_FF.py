...
try:
    ev_name = "UNIFORM1"
    hyp_time, lat_ev, lon_ev, dep, mxx, myy, mzz, mxy, mxz, myz = hls.read_finitefaultmodel(infile="./DF_data/"+ev_name+".txt")
    x_ev,y_ev = hls.convert_to_UTM(lon=lon_ev, lat=lat_ev, zone=33)
    src = [];
    for k in range(0,len(hyp_time)):
        elv = topo.ds.sel(x=x_ev[k], y=y_ev[k], method='nearest')

        src1 = sn.simple_config.source.cartesian.SideSetMomentTensorPoint3D(
        ...
        src.append(src1)
    p.add_to_project(sn.Event(event_name=ev_name, sources=src, receivers=recs))
except Exception as e:
...
p.simulations.launch(
    ...
    wall_time_in_seconds_per_job=3600*48, #3600 24h
